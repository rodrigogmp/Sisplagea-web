    ...
    <v-container fluid>
        <v-layout justify-center>
            <v-flex xs12 sm10 md8 lg6>
                <h1>Projetos de Pesquisa Cadastrados</h1>
                <v-divider></v-divider>
                <v-data-iterator :items="projetos" :rows-per-page-items="rowsPerPageItems" :pagination.sync="pagination" content-tag="v-layout" column>
                    <template v-slot:item="props">
                                                ...
                                                <v-list-tile-action class="align-end">
                                                    <v-tooltip bottom>
                                                        <template v-slot:activator="{ on }">
                                                            <v-btn flat v-on="on" @click="dialog = true"><v-icon color="green lighten-1">info</v-icon>Info Projeto</v-btn>
                                                        </template>
                        ...
                        <v-dialog v-model="dialog" persistent max-width="680px">
                                <v-card>
                                    <v-card-title>
                                        <span class="headline">Projeto: {{ props.item.name }} </span>
                                    </v-card-title>
                                    <v-card-text>
...
export default {
    ...
    mounted() {
        axios
            .get('https://sisplagea-api.herokuapp.com/api/v1/projects.json', config)
            .then((response) => {
